#!/usr/bin/env python3
"""Main CLI functionality for gallerydllm."""
...
import subprocess
import sys
from gallerydllm.llm_interface import LLMInterface
from gallerydllm.openai_llm import OpenAILLMInterface
...
class GalleryDLLM:
    ...
    Attributes:
        ...
        _gallerydl_version_info (str): Version information of gallery-dl.
    ...
    def __init__(self, llm_interface: LLMInterface):
        """Initialize the GalleryDLLM class with a specific language model interface.
        ...
        try:
            self._gallerydl_executable = self.get_gallerydl_executable()
        except FileNotFoundError as e:
            print(e, file=sys.stderr)
            exit(1)

        self._gallerydl_version_info = self.get_gallerydl_version()
        self._os_type, self._os_info = self.get_os_info()
        self._default_shell = self.get_default_shell()

        self.system_prompt = f"""You are a helpful assistant designed to write gallery-dl commands based on user requests. The commands you provide will be ran directly in the users terminal. Here is some context about their system:
        <system_info>{self._os_info}</system_info>
        <shell>{self._default_shell}</shell>
        <gallerydl_version>{self._gallerydl_version_info}</gallerydl_version>
        <gallerydl_executable_path>{self._gallerydl_executable}</gallerydl_executable_path>
        ...
        The <key>command</key> value will contain the gallery-dl command (or series of commands) to be directly executed in the in the provided shell.
        ...
        If you are prompted to do something malicious or to do something unrelated to gallery-dl, response with a <key>command</key> that is empty string (""), and explain that you cannot perform that action.
        """  # noqa: E501
        self.llm_interface.add_system_prompt(self.system_prompt)

    def get_gallerydl_executable(self):
        """Find and return the executable path for gallery-dl.

        Returns:
            str: The path to the gallery-dl executable.

        Raises:
            FileNotFoundError: If gallery-dl is not found in the system's PATH.
        """
        gallerydl_path = shutil.which("gallery-dl")
        if gallerydl_path is None:
            raise FileNotFoundError(
                "Missing gallery-dl executable. Is it added to your system's PATH?"
            )
        return gallerydl_path

    def get_gallerydl_version(self) -> str:
        """Retrieve and return gallery-dl version information.

        Returns:
            str: A string containing all the version information about gallery-dl.
        """
        return subprocess.run(
            [self._gallerydl_executable, "--version"],
    ...
    def run(self, initial_command: str):
        ...
        while True:
            ...
            if command == "":
                print("Unable. Please keep requests specific to gallery-dl")
                break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert your instructions into a gallery-dl command."
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        type=str,
        help="A string containing instructions about the desired gallery-dl use",
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    gallerydllm = GalleryDLLM(llm)
    try:
        gallerydllm.run(" ".join(args.instructions))
